...
class SearchRecodeAdapter(private val recodes: List<SearchRecord>): RecyclerView.Adapter<SearchRecodeAdapter.ViewHolder>() {

    // 뷰 유형에 대한 참조 클래스
    class ViewHolder(val binding: ItemSearchBinding) : RecyclerView.ViewHolder(binding.root) {
        fun setItem(recodes: String) {
    ...
    override fun onBindViewHolder(viewHolder: ViewHolder, position: Int) {
        viewHolder.setItem(recodes[position].searchRecode)
    }

    // 데이터 크기 반환
    override fun getItemCount() = recodes.size
